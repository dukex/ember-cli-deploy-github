...
module.exports = CoreObject.extend({
  ...
  upload: function(distDir) {
    var repo, index, signature, remote;
    ...
    return git.Repository.init(distDir, isBare)
      ...
      .then(function(oid) {
        ...
        remote  = _remote;
        return repo.fetchAll(self._gitAuthCallbacks(git));
      }).then(function(){
        return repo.rebaseBranches("master", "origin/" + branch);
      }).then(function() {
        remote.setCallbacks(self._gitAuthCallbacks(git));
  ...
  _gitAuthCallbacks: function(git) {
    return {
