...
module.exports = CoreObject.extend({
  init: function(git, config) {
    this.path = path.resolve(this.distDir || "./tmp/deploy-dist");
  ...
  upload: function() {
    ...
    return this._initRepository()
      ...
      .then(function(oid) {
        ...
        return _this.git.Remote.create(repo, 'origin', _this.repository);
      }).then(function(remote) {
        return remote ? remote : _this.git.Remote.lookup(repo, 'origin');
      }).then(function(remote) {
        remote.setCallbacks({
          credentials: function(url, userName) {
            return _this.git.Cred.sshKeyFromAgent(userName);
          }
        });
